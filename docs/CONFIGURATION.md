...
| `label_direction`                  | string       | `"reverse"`             | Hint label algorithm: `"reverse"` (reverse-order tiers) or `"normal"` (prefix-avoidance greedy). Empty value defaults to `"reverse"`. Overridable per-app via `[hints.app_configs]` and per-activation via the `neru hints --label-direction` CLI flag.                                                          |
