...
func TestBuildStyle_UsesThemeAwareConfigValues(t *testing.T) {
	...
	if lightStyle.BackgroundColor() != config.HintsBackgroundColorLight {
		t.Fatalf("expected light background %q, got %q", config.HintsBackgroundColorLight, lightStyle.BackgroundColor())
	}
	if lightStyle.TextColor() != config.HintsTextColorLight {
		t.Fatalf("expected light text %q, got %q", config.HintsTextColorLight, lightStyle.TextColor())
	}
	if lightStyle.MatchedTextColor() != config.HintsMatchedTextColorLight {
		t.Fatalf("expected light matched text %q, got %q", config.HintsMatchedTextColorLight, lightStyle.MatchedTextColor())
	}
	if lightStyle.BorderColor() != config.HintsBorderColorLight {
		t.Fatalf("expected light border %q, got %q", config.HintsBorderColorLight, lightStyle.BorderColor())
	}

	darkStyle := BuildStyle(cfg, &mockThemeProvider{darkMode: true})
	if darkStyle.BackgroundColor() != config.HintsBackgroundColorDark {
		t.Fatalf("expected dark background %q, got %q", config.HintsBackgroundColorDark, darkStyle.BackgroundColor())
	}
	...
	if darkStyle.MatchedTextColor() != config.HintsMatchedTextColorDark {
		t.Fatalf("expected dark matched text %q, got %q", config.HintsMatchedTextColorDark, darkStyle.MatchedTextColor())
	}
	if darkStyle.BorderColor() != config.HintsBorderColorDark {
		t.Fatalf("expected dark border %q, got %q", config.HintsBorderColorDark, darkStyle.BorderColor())
	}
}
