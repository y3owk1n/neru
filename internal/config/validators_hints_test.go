package config_test

import (
	"testing"

	"github.com/y3owk1n/neru/internal/config"
)

// TestValidateHints tests the validateHints function.
func TestValidateHints(t *testing.T) {
	tests := []struct {
		name    string
		config  config.Config
		wantErr bool
	}{
		{
			name: "valid hints config",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					MaxDepth:              50,
					ParallelThreshold:     20,
					AdditionalAXSupport: config.AdditionalAXSupport{
						AdditionalElectronBundles: []string{"com.example.app"},
						AdditionalChromiumBundles: []string{"com.example.chromium"},
						AdditionalFirefoxBundles:  []string{"com.example.firefox"},
					},
				},
			},
			wantErr: false,
		},
		{
			name: "valid hints config with auto padding and border radius",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          -1,
					PaddingX:              -1,
					PaddingY:              -1,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					MaxDepth:              50,
					ParallelThreshold:     20,
				},
			},
			wantErr: false,
		},
		{
			name: "valid hints config with unlimited max_depth",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					MaxDepth:              0,
					ParallelThreshold:     20,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: false,
		},
		{
			name: "negative max_depth",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					MaxDepth:              -1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "empty hint_characters",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "hint_characters too short",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "a",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "invalid background_color",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "invalid",
					TextColorLight:        "#000000",
					MatchedTextColorLight: "#FF0000",
					BorderColorLight:      "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "invalid text_color",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					TextColorLight:        "invalid",
					MatchedTextColorLight: "#FF0000",
					BorderColorLight:      "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "invalid matched_text_color",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					TextColorLight:        "#000000",
					MatchedTextColorLight: "invalid",
					BorderColorLight:      "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "invalid border_color",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					TextColorLight:        "#000000",
					MatchedTextColorLight: "#FF0000",
					BorderColorLight:      "invalid",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "negative font_size",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              -1,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "zero font_size",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              0,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "border_radius below -1",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          -2,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "padding_x below -1",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              -2,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "padding_y below -1",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              -2,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "negative border_width",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           -1,
					ClickableRoles:        []string{"AXButton"},
				},
			},
			wantErr: true,
		},
		{
			name: "zero parallel_threshold",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					ParallelThreshold:     0,
				},
			},
			wantErr: true,
		},
		{
			name: "valid hints config with auto_exit_actions",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					MaxDepth:              50,
					ParallelThreshold:     20,
					AutoExitActions:       []string{"left_click", "right_click"},
				},
			},
			wantErr: false,
		},
		{
			name: "valid hints config with empty auto_exit_actions",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					MaxDepth:              50,
					ParallelThreshold:     20,
					AutoExitActions:       []string{},
				},
			},
			wantErr: false,
		},
		{
			name: "valid hints config with all direct key binding actions in auto_exit_actions",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					MaxDepth:              50,
					ParallelThreshold:     20,
					AutoExitActions: []string{
						"left_click",
						"right_click",
						"middle_click",
						"mouse_down",
						"mouse_up",
						"move_mouse_relative",
					},
				},
			},
			wantErr: false,
		},
		{
			name: "invalid hints auto_exit_actions with unknown action",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					MaxDepth:              50,
					ParallelThreshold:     20,
					AutoExitActions:       []string{"left_click", "unknown_action"},
				},
			},
			wantErr: true,
		},
		{
			name: "invalid hints auto_exit_actions with scroll (IPC-only)",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					MaxDepth:              50,
					ParallelThreshold:     20,
					AutoExitActions:       []string{"scroll"},
				},
			},
			wantErr: true,
		},
		{
			name: "invalid hints auto_exit_actions with move_mouse (IPC-only)",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ClickableRoles:        []string{"AXButton"},
					MaxDepth:              50,
					ParallelThreshold:     20,
					AutoExitActions:       []string{"move_mouse"},
				},
			},
			wantErr: true,
		},
		{
			name: "empty string in clickable_roles",
			config: config.Config{
				Hints: config.HintsConfig{
					HintCharacters:        "abcd",
					BackgroundColorLight:  "#FFFFFF",
					BackgroundColorDark:   "#FFFFFF",
					TextColorLight:        "#000000",
					TextColorDark:         "#000000",
					MatchedTextColorLight: "#FF0000",
					MatchedTextColorDark:  "#FF0000",
					BorderColorLight:      "#000000",
					BorderColorDark:       "#000000",
					FontSize:              12,
					BorderRadius:          4,
					PaddingX:              4,
					PaddingY:              4,
					BorderWidth:           1,
					ParallelThreshold:     20,
					ClickableRoles:        []string{"AXButton", ""},
				},
			},
			wantErr: true,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := tt.config.ValidateHints()
			if (err != nil) != tt.wantErr {
				t.Errorf("ValidateHints() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
