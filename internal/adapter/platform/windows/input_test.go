...
func TestAbsoluteCoordinate_SpansTheVirtualDesktop(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:   "last pixel maps to the top of the range",
			pixel:  1919,
			origin: -1920,
			size:   3840,
			want:   65535,
		...
		{
			name:   "pixel before the desktop clamps to zero",
			pixel:  -5000,
			origin: -1920,
			size:   3840,
			want:   0,
		...
		{
			name:   "pixel past the desktop clamps to the top",
			pixel:  9000,
			origin: -1920,
			size:   3840,
			want:   65535,
		...
		{
			name:   "degenerate desktop maps to zero",
			pixel:  10,
			origin: 0,
			size:   1,
			want:   0,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if got := absoluteCoordinate(test.pixel, test.origin, test.size); got != test.want {
				t.Fatalf(
					"absoluteCoordinate(%d, %d, %d) = %d, want %d",
					test.pixel, test.origin, test.size, got, test.want,
				)
			}
		})
	}
