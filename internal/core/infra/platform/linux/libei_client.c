...
static void drain(NeruEiClient *c) {
	...
	while ((e = ei_get_event(c->ei)) != NULL) {
		...
		case EI_EVENT_DEVICE_ADDED: {
			...
			if (!c->pointer &&
			    ei_device_has_capability(d, EI_DEVICE_CAP_POINTER_ABSOLUTE)) {
				c->pointer = ei_device_ref(d);
			}
			if (!c->keyboard &&
			    ei_device_has_capability(d, EI_DEVICE_CAP_KEYBOARD)) {
...
static int ensure_emulating(NeruEiClient *c, struct ei_device *device,
                            int resumed, int *emulating) {
	if (!device || !resumed) {
		return 0;
	}
	if (!*emulating) {
...
NeruEiClient *neru_ei_connect(int timeout_ms) {
	...
	oeffis_create_session(c->oeffis,
	                      OEFFIS_DEVICE_POINTER | OEFFIS_DEVICE_KEYBOARD);
	...
	while (eis_fd < 0) {
		...
		} else if (ev == OEFFIS_EVENT_DISCONNECTED ||
		           ev == OEFFIS_EVENT_CLOSED) {
			neru_ei_disconnect(c);
			return NULL;
		}
	}
...
int neru_ei_move_abs(NeruEiClient *c, int x, int y) {
	...
	pump(c);
	if (!ensure_emulating(c, c->pointer, c->pointer_resumed,
	                      &c->pointer_emulating)) {
		return 0;
	}
...
int neru_ei_key(NeruEiClient *c, int keycode, int pressed) {
	...
	pump(c);
	if (!ensure_emulating(c, c->keyboard, c->keyboard_resumed,
	                      &c->keyboard_emulating)) {
		return 0;
	}
