...
int neru_x11_query_pointer(Display *display, int *x, int *y) {
  ...
  return XQueryPointer(display, root, &root_return, &child_return, x, y, &win_x,
                       &win_y, &mask_return);
}
...
int neru_x11_get_active_window(Display *display, Window *out) {
  ...
  Window root = neru_x11_root_window(display);
  int status = XGetWindowProperty(display, root, property, 0, 1, False,
...
unsigned long neru_x11_get_window_pid(Display *display, Window window,
                                      int *ok) {
  if (window == 0) {
    *ok = 0;
    return 0;
...
NeruX11Monitor *neru_x11_get_monitors(Display *display, int *count) {
  ...
  NeruX11Monitor *result =
      calloc((size_t)monitor_count, sizeof(NeruX11Monitor));
  if (result == NULL) {
