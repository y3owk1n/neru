package quadgrid

import (
	"image"
	"testing"

	"github.com/stretchr/testify/assert"
)

func TestNewQuadGrid(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	assert.Equal(t, bounds, qg.currentBounds, "Current bounds should match initial bounds")
	assert.Equal(t, bounds, qg.initialBounds, "Initial bounds should be stored")
	assert.Equal(t, 0, qg.depth, "Initial depth should be 0")
	assert.Equal(t, 25, qg.minSize, "Min size should be set")
	assert.Equal(t, 10, qg.maxDepth, "Max depth should be set")
}

func TestDivide(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	quadrants := qg.Divide()

	// Verify 4 quadrants
	assert.Len(t, quadrants, 4, "Should have 4 quadrants")

	// Verify top-left quadrant
	expectedTL := image.Rect(0, 0, 50, 50)
	assert.Equal(t, expectedTL, quadrants[TopLeft], "Top-left quadrant should be correct")

	// Verify top-right quadrant
	expectedTR := image.Rect(50, 0, 100, 50)
	assert.Equal(t, expectedTR, quadrants[TopRight], "Top-right quadrant should be correct")

	// Verify bottom-left quadrant
	expectedBL := image.Rect(0, 50, 50, 100)
	assert.Equal(t, expectedBL, quadrants[BottomLeft], "Bottom-left quadrant should be correct")

	// Verify bottom-right quadrant
	expectedBR := image.Rect(50, 50, 100, 100)
	assert.Equal(t, expectedBR, quadrants[BottomRight], "Bottom-right quadrant should be correct")
}

func TestSelectQuadrant(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	// Select top-left
	center, completed := qg.SelectQuadrant(TopLeft)

	expectedCenter := image.Point{X: 25, Y: 25}
	assert.Equal(t, expectedCenter, center, "Center should be at (25, 25)")
	assert.False(t, completed, "Should not be completed after first selection")

	// Verify depth increased
	assert.Equal(t, 1, qg.depth, "Depth should be 1")

	// Verify bounds narrowed
	expectedBounds := image.Rect(0, 0, 50, 50)
	assert.Equal(
		t,
		expectedBounds,
		qg.currentBounds,
		"Bounds should be narrowed to top-left quadrant",
	)
}

func TestSelectQuadrantCompletion(t *testing.T) {
	// Create a small grid where one selection reaches minimum size
	// With bounds 50x50 and minSize 25:
	// - First division creates 25x25 quadrants
	// - 25/2 = 12 < 25, so CanDivide returns false after first selection
	bounds := image.Rect(0, 0, 50, 50)
	qg := NewQuadGrid(bounds, 25, 10)

	// Select top-left - should complete since resulting quadrant (25x25) cannot be divided further
	center, completed := qg.SelectQuadrant(TopLeft)

	// After selecting top-left, bounds are (0,0)-(25,25), center is at (12, 12)
	expectedCenter := image.Point{X: 12, Y: 12}
	assert.Equal(t, expectedCenter, center, "Center should be at (12, 12)")
	assert.True(t, completed, "Should be completed when min size is reached")
}

func TestCanDivide(t *testing.T) {
	tests := []struct {
		name     string
		bounds   image.Rectangle
		minSize  int
		expected bool
	}{
		{
			name:     "Can divide 100x100 with min 25",
			bounds:   image.Rect(0, 0, 100, 100),
			minSize:  25,
			expected: true,
		},
		{
			name:     "Cannot divide 40x40 with min 25",
			bounds:   image.Rect(0, 0, 40, 40),
			minSize:  25,
			expected: false, // 40/2 = 20 < 25
		},
		{
			name:     "Can divide 50x50 with min 25",
			bounds:   image.Rect(0, 0, 50, 50),
			minSize:  25,
			expected: true, // 50/2 = 25 >= 25
		},
		{
			name:     "Cannot divide when max depth reached",
			bounds:   image.Rect(0, 0, 1000, 1000),
			minSize:  1,
			expected: false, // depth starts at 0, max is 10
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			qg := NewQuadGrid(tt.bounds, tt.minSize, 10)
			// Simulate max depth for the last test
			if tt.name == "Cannot divide when max depth reached" {
				qg.depth = 10
			}

			result := qg.CanDivide()
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestBacktrack(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	// Make a selection
	qg.SelectQuadrant(TopLeft)

	// Backtrack
	result := qg.Backtrack()

	assert.True(t, result, "Backtrack should return true")
	assert.Equal(t, bounds, qg.currentBounds, "Bounds should be restored to original")
	assert.Equal(t, 0, qg.depth, "Depth should be 0")

	// Backtrack again should fail (no history)
	result = qg.Backtrack()
	assert.False(t, result, "Backtrack should return false when no history")
}

func TestReset(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	// Make some selections
	qg.SelectQuadrant(TopLeft)
	qg.SelectQuadrant(BottomRight)

	// Reset
	qg.Reset()

	assert.Equal(t, bounds, qg.currentBounds, "Bounds should be restored to initial")
	assert.Equal(t, 0, qg.depth, "Depth should be 0")
	assert.Empty(t, qg.history, "History should be cleared")
}

func TestCurrentCenter(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	center := qg.CurrentCenter()
	expected := image.Point{X: 50, Y: 50}
	assert.Equal(t, expected, center, "Center should be at (50, 50)")

	// Select a quadrant and check new center
	qg.SelectQuadrant(TopLeft)
	center = qg.CurrentCenter()
	expected = image.Point{X: 25, Y: 25}
	assert.Equal(t, expected, center, "Center should be at (25, 25) after selecting top-left")
}

func TestQuadrantBounds(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	tl := qg.QuadrantBounds(TopLeft)
	expected := image.Rect(0, 0, 50, 50)
	assert.Equal(t, expected, tl, "Top-left bounds should be correct")

	br := qg.QuadrantBounds(BottomRight)
	expected = image.Rect(50, 50, 100, 100)
	assert.Equal(t, expected, br, "Bottom-right bounds should be correct")
}

func TestQuadrantCenter(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	center := qg.QuadrantCenter(TopRight)
	expected := image.Point{X: 75, Y: 25}
	assert.Equal(t, expected, center, "Top-right center should be at (75, 25)")
}

func TestIsComplete(t *testing.T) {
	bounds := image.Rect(0, 0, 100, 100)
	qg := NewQuadGrid(bounds, 25, 10)

	assert.False(t, qg.IsComplete(), "Should not be complete initially")

	// Select until we can't divide anymore
	for qg.CanDivide() {
		qg.SelectQuadrant(TopLeft)
	}

	assert.True(t, qg.IsComplete(), "Should be complete when CanDivide returns false")
}
